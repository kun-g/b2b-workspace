...
- [ ] 创建 `lease-shelf-flow-01487/e2e/tests/credit/merchant-credit-management.spec.ts`
- [ ] 测试场景：
  - 商户管理员登录
  - 查看授信列表
  - 添加新授信（选择用户、输入额度）
...
**优化方向**：
- [ ] 确认当前提示是否已足够清晰（根据你的反馈，设备押金红色字体已实现）
- [ ] 如需优化，可调整为：`授信额度不足：您当前可用额度为 ¥XXX，下单需要 ¥XXX，还差 ¥XXX`
- [ ] 添加操作建议：`请联系商户提高授信额度`
...
**实现方案**：
- [ ] 在授信详情弹窗或折叠面板中显示历史记录
- [ ] 显示格式：`YYYY-MM-DD HH:mm - 操作人: 额度从 ¥XXX 调整为 ¥XXX（原因）`
...
### 测试验证
- [ ] 单元测试全部通过：`pnpm test:int creditUtils`
- [ ] 集成测试全部通过：`pnpm test:int Orders`
- [ ] E2E 测试全部通过：`pnpm test:e2e`

### 代码质量
- [ ] 前端无编译错误：`cd lease-shelf-flow-01487 && pnpm build`
- [ ] 后端无编译错误：`cd b2b-rental-backend && pnpm build`
